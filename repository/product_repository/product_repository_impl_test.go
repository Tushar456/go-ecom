...
func TestCreateProduct(t *testing.T) {
	...
	tcs := []struct {
		...
		{
			name: "success",
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				cp, err := ms.CreateProduct(context.Background(), p)
...

		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				_, err := ms.CreateProduct(context.Background(), p)
				require.Error(t, err)
				err = mock.ExpectationsWereMet()
				require.NoError(t, err)
			},
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			withTestDB(t, func(db *sqlx.DB, mock sqlmock.Sqlmock) {
				st := NewProductRepositoryImpl(db)
				tc.test(t, st, mock)
			})
		})
	}
	...
	tcs := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				gp, err := ms.GetProduct(context.Background(), 1)
				require.NoError(t, err)
...

		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				_, err := ms.GetProduct(context.Background(), 1)
				require.Error(t, err)
...
func TestListProducts(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "image", "category", "description", "rating", "num_reviews", "price", "count_in_stock", "created_at", "updated_at"}).
		AddRow(1, p.Name, p.Image, p.Category, p.Description, p.Rating, p.NumReviews, p.Price, p.CountInStock, p.CreatedAt, p.UpdatedAt).
		AddRow(1, p2.Name, p2.Image, p2.Category, p2.Description, p2.Rating, p2.NumReviews, p2.Price, p2.CountInStock, p2.CreatedAt, p2.UpdatedAt)
	tcs := []struct {
		name string
		test func(*testing.T, ProductRepository, sqlmock.Sqlmock)
	}{
		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				gp, err := ms.ListProducts(context.Background())
				require.NoError(t, err)
				require.Equal(t, 2, len(gp))
...

		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				_, err := ms.ListProducts(context.Background())
				require.Error(t, err)
	...
	tcs := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				up, err := ms.UpdateProduct(context.Background(), np)
				require.NoError(t, err)
...

		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				_, err := ms.UpdateProduct(context.Background(), np)
				require.Error(t, err)
			},
		},
	}
	...
	tcs := []struct {
	...
	}{
		{
			...
			test: func(t *testing.T, ms ProductRepository, mock sqlmock.Sqlmock) {
				...
				err := ms.DeleteProduct(context.Background(), 1)
				require.NoError(t, err)
				err = mock.ExpectationsWereMet()
				require.NoError(t, err)
